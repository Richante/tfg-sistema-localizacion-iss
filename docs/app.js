...
const API_URL = parametros.get("api");
const STATIC_BASE = parametros.get("static") || "static";
const modoEstatico = !API_URL;

const rutasEstaticas = {
    "/api/estado": "estado.json",
    "/api/trayectoria": "trayectoria.json",
    "/api/resultados": "resultados.json",
    "/api/trayectorias-comparadas": "trayectorias-comparadas.json"
};
...
async function cargarJson(ruta) {
    const url = API_URL
        ? `${API_URL}${ruta}`
        : `${STATIC_BASE}/${rutasEstaticas[ruta]}`;
...
async function iniciar() {
    try {
        ...
        const etiquetaModo = modoEstatico ? "demo estatica" : "backend";
        setEstado(`${estado.servicio}: ${estado.estado} (${etiquetaModo})`, "ok");
        ...
        console.error(error);
        setEstado("No se pudieron cargar los datos", "error");
    }
}
